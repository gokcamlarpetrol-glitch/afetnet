...
export async function buildQuickBackup(password:string){
  ...
  const raw = new (globalThis as any).TextEncoder().encode(JSON.stringify(obj));
  const enc = await secretboxEncrypt(raw, password);
  const payload = JSON.stringify(enc);
  const parts = chunk((globalThis as any).Buffer.from(payload).toString('base64'), 800);
...
export async function restoreQuickBackup(chunks: Chunk[], password: string){
  ...
  const json = (globalThis as any).Buffer.from(b64, 'base64').toString();
  const dec = await secretboxDecrypt(JSON.parse(json), password);
  const obj = JSON.parse(new (globalThis as any).TextDecoder().decode(dec));
