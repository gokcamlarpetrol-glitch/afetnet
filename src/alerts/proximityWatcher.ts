...
async function beep(){
  try{
    ...
    await sound.playAsync(); (globalThis as any).setTimeout(()=>sound.unloadAsync().catch(()=>{}), 500);
  }catch{
    // Ignore audio errors
  }
}
...
export function startProximityWatcher(getLastLocation?:()=>Promise<{latitude:number;longitude:number}|null>){
  ...
  async function loop(){
    ...
    t = (globalThis as any).setTimeout(loop, paramsFor(useProfile.getState().profile).proximityPollMs) as any;
  }
  loop();
}

export function stopProximityWatcher(){ if(t){ (globalThis as any).clearTimeout(t); t=null; } }
