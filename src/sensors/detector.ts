...
export async function startDetector(){
  ...
  async function cycle(){
    if(!useSafety.getState().sensorDetect){ dutyTimer = (globalThis as any).setTimeout(cycle, sleepSec*1000); return; }
    ...
    dutyTimer = (globalThis as any).setTimeout(cycle, sleepSec*1000);
  }
  cycle();
}

export function stopDetector(){ running=false; if(dutyTimer){ (globalThis as any).clearTimeout(dutyTimer); dutyTimer=null; } }
