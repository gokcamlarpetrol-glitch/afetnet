/**
 * EARTHQUAKE STORE TESTS - ELITE EDITION
 * Unit tests for earthquake state management
 */
...
describe('EarthquakeStore', () => {
    ...
    test('setEarthquakes should update earthquakes array', () => {
        ...
        const state = useEarthquakeStore.getState();
        expect(state.earthquakes).toHaveLength(2);
        expect(state.earthquakes[0].magnitude).toBe(4.5);
        expect(state.earthquakes[1].location).toBe('Ankara');
    });
    ...
    test('should filter earthquakes by magnitude', () => {
        ...
        const state = useEarthquakeStore.getState();
        const significantEarthquakes = state.earthquakes.filter(e => e.magnitude >= 4.0);
