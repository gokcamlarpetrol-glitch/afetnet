...
export async function getOwn(){
  ...
  const obj = { publicKey: Buffer.from(kp.publicKey).toString("base64"), secretKey: Buffer.from(kp.secretKey).toString("base64") };
...
export async function sealFor(peerPubB64: string, msg: Uint8Array){
  ...
  const sk = Buffer.from(own.secretKey,"base64");
  const nonce = nacl.randomBytes(24);
  const boxed = nacl.box(msg, nonce, pub, sk);
  return Buffer.concat([Buffer.from(nonce), Buffer.from(boxed)]);
}

export async function openFrom(senderPubB64: string, blob: Uint8Array){
  const own = await getOwn();
  const pub = Buffer.from(senderPubB64,"base64");
  const sk = Buffer.from(own.secretKey,"base64");
