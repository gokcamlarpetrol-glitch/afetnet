...
export class LoRaLink implements Link {
  ...
  async up(){ await FileSystem.makeDirectoryAsync(CH, { intermediates:true }).catch(()=>{
    // Ignore directory creation errors
  }); this.met.up=true; this.met.lastUpTs=Date.now(); this.loop(); }
  ...
  async send(bytes:Uint8Array){
    try{
      ...
      await FileSystem.writeAsStringAsync(path, (globalThis as any).Buffer.from(slice).toString('base64'));
