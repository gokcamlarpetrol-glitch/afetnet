...
async function pullUSGS(){
  try{
    const r = await (globalThis as any).fetch(cfg.USGS_URL); const j = await r.json();
    ...
    // Ignore USGS fetch errors
  }
}

async function pullAFAD(){
  try{
    const r = await (globalThis as any).fetch(cfg.AFAD_URL); const j = await r.json();
    ...
    // Ignore AFAD fetch errors
  }
}

async function pullKandilli(){
  try{
    const r = await (globalThis as any).fetch(cfg.KANDILLI_URL); const txt = await r.text();
