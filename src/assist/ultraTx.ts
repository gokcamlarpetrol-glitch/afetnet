...
function make19kWavBase64(){
  ...
  // @ts-ignore
  return (typeof (globalThis as any).btoa !== 'undefined') ? (globalThis as any).btoa(bin) : (globalThis as any).Buffer.from(bin,'binary').toString('base64');
}
...
export async function stopUltraTx(){ 
  on=false; 
  try{ 
