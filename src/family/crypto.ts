...
function xor(k:string, s:string){ const kb=new TextEncoder().encode(k), sb=new TextEncoder().encode(s); const out=new Uint8Array(sb.length); for(let i=0;i<sb.length;i++){ out[i]=sb[i] ^ kb[i%kb.length]; } return new TextDecoder().decode(out); }