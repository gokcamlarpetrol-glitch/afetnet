...
export default function E2EESetupScan({ route, navigation }:any){
  ...
  async function onScan({ data }:any){
    try{
      ...
      const { pre_sk, pre_pk } = await ensureIdentity();
      const threadId = await threadFromRV(rv);
      await createSessionFromBundles(threadId, { sk: pre_sk, pk: pre_pk }, { ik_pub: new Uint8Array((globalThis as any).Buffer.from(peer.ik_pub_b64,'base64')), spk_pub: new Uint8Array((globalThis as any).Buffer.from(peer.spk_pub_b64,'base64')) });
