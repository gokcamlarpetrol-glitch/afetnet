// MAP SCREEN - FULLY FUNCTIONAL WITH OFFLINE SUPPORT
// Production-ready map screen with SOS signals, trapped people, and offline map support
import React, { useEffect, useState, useRef } from 'react';
import { View, Text, useWindowDimensions, Pressable, Alert, Modal, ActivityIndicator, StyleSheet } from 'react-native';
...
import { readSOS } from '../match/compute';
import { readInbox } from '../p2p/inboxReader';
...
type SOSSignal = { id: string; lat: number; lng: number };
...
export default function MapScreen(){
  ...
  // SOS signals from offline inbox
  const [sosSignals, setSosSignals] = useState<SOSSignal[]>([]);
  const [inboxSignals, setInboxSignals] = useState<any[]>([]);
  
  const mapRef = useRef<any>(null);

  // Premium check - show lock overlay if not premium
  if (!canUseFeature('offline_maps')) {
    return (
      <View style={{ flex: 1, backgroundColor: '#0f172a', justifyContent: 'center', alignItems: 'center', padding: 20 }}>
        <Text style={{ color: '#f59e0b', fontSize: 24, fontWeight: '800', marginBottom: 16, textAlign: 'center' }}>
          🔒 Premium Gerekli
        </Text>
        ...
        <Pressable
          ...
          onPress={() => {
            // Navigation will be handled by RootTabs LockedOverlay
          }}
        >
          <Text style={{ color: '#fff', fontSize: 16, fontWeight: '700' }}>Premium Satın Al</Text>
        </Pressable>
      </View>
    );
  }
  ...
  const updateSOSSignals = async () => {
    try {
      // Get SOS signals from inbox
      const sos = await readSOS();
      setSosSignals(sos);

      // Get inbox signals (includes SOS and status updates)
      const inbox = await readInbox();
      setInboxSignals(inbox.filter(s => s.kind === 'sos' && s.qlat && s.qlng));

      logger.debug(`MapScreen - SOS signals updated: ${sos.length} SOS signals`);
    } catch (error) {
      logger.error('Failed to update SOS signals:', error);
    }
  };
  ...
  useEffect(() => { 
    refreshPins(); 
    refreshTrail(2); 
    const t = (globalThis as any).setInterval(() => refreshTrail(2), 60000); 
  ...
  if(!ExpoMap){
    return (
      <View style={{ flex:1, backgroundColor:'#0f172a' }}>
        <Text style={{ color:'#94a3b8', padding:12 }}>Harita modülü devre dışı. expo-maps gerekli.</Text>
        <View style={{ flex:1 }}/>
        <Pressable onPress={onImportMbtiles} style={{ backgroundColor:'#1f2937', padding:14, margin:12, borderRadius:12 }} disabled={isLoading}>
          {isLoading ? (
  ...
  return (
    <View style={{ flex:1 }}>
      ...
      >
        ...
        {/* SOS Signals from Inbox - CRITICAL: Show trapped people */}
        {sosSignals.map((sos) => (
          <ExpoMap.Marker
            key={`sos-${sos.id}`}
            coordinate={{ latitude: sos.lat, longitude: sos.lng }}
            title="🚨 SOS SİNYALİ"
            description="Enkaz altında veya yardıma ihtiyacı olan kişi"
            pinColor="red"
            onPress={() => {
              Alert.alert(
                '🚨 SOS SİNYALİ TESPİT EDİLDİ',
                `SOS sinyali alındı!\n\nKonum: ${sos.lat.toFixed(6)}, ${sos.lng.toFixed(6)}\nID: ${sos.id}\n\nAcil müdahale gerekiyor!`,
                [
                  { text: 'İptal', style: 'cancel' },
                  {
                    text: 'Oraya Git',
                    onPress: () => {
                      setSelectedTarget({ lat: sos.lat, lon: sos.lng, label: 'SOS Sinyali' });
                      setShowGoToTarget(true);
                    },
                  },
                ]
        ...
        {/* Inbox Signals (SOS from P2P) */}
        {inboxSignals.map((signal) => (
          <ExpoMap.Marker
            key={`inbox-sos-${signal.id}`}
            coordinate={{ latitude: signal.qlat!, longitude: signal.qlng! }}
            title="🚨 ACİL DURUM"
            description={`${new Date(signal.ts).toLocaleString()} - P2P Mesaj`}
            pinColor="red"
            onPress={() => {
              Alert.alert(
                '🚨 ACİL DURUM SİNYALİ',
                `P2P mesajından gelen acil durum sinyali!\n\nKonum: ${signal.qlat!.toFixed(6)}, ${signal.qlng!.toFixed(6)}\nZaman: ${new Date(signal.ts).toLocaleString()}\nID: ${signal.id}`,
                [
                  { text: 'İptal', style: 'cancel' },
                  {
                    text: 'Oraya Git',
                    onPress: () => {
                      setSelectedTarget({ lat: signal.qlat!, lon: signal.qlng!, label: 'Acil Durum' });
                      setShowGoToTarget(true);
                    },
                  },
                ]
      ...
      {/* Enhanced Controls */}
      <Pressable onPress={onImportMbtiles} style={{ position:'absolute', right:16, top:64, backgroundColor:'#111827', padding:10, borderRadius:10 }} disabled={isLoading}>
        {isLoading ? (
      ...
      <View style={{ position:'absolute', top:60, left:16, backgroundColor:'rgba(17, 24, 39, 0.9)', padding:12, borderRadius:10, gap:4 }}>
        ...
        <Text style={{ color:'#ef4444', fontSize:12, fontWeight:'bold' }}>🚨 {sosSignals.length + inboxSignals.length} SOS Sinyali</Text>
        {!isOnline && (
          <>
