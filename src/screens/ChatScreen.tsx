...
export default function ChatScreen({ route, navigation }:any){
  ...
  useEffect(()=>{ const t = (globalThis as any).setInterval(refresh, 4000); refresh(); return ()=>(globalThis as any).clearInterval(t); },[]);
  ...
  async function send(kind:'text'|'sos'='text'){
    ...
    if(secure){
      try{
        ...
        // If no session, fallback to plain text
      }
    }
    let q:any={};
    try{
    ...
    setText('');
    (globalThis as any).setTimeout(refresh, 600);
  }
