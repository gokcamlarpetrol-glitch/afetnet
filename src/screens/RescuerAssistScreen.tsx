...
export default function RescuerAssistScreen(){
  ...
  useEffect(()=>{ const t=(globalThis as any).setInterval(async()=>{ await findNearest(); }, 8000); findNearest(); return ()=>(globalThis as any).clearInterval(t); },[]);
  ...
  async function guide(){
    ...
    (async function loop(){
      while(on){
        ...
        await new Promise(r=>(globalThis as any).setTimeout(r, d<50? 2000 : d<150? 4000 : 7000));
      }
    })();
    // stop when unmount
    return ()=>{ on=false; };
