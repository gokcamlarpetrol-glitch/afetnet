...
(globalThis as any).__AFN_BLE_RESTART__ = ()=>{ /* no-op placeholder; could reinit BLE manager here */ };
...
function seen(id:any){ const s=String(id); if(seenIds.has(s)) {return true;} seenIds.add(s); (globalThis as any).setTimeout(()=>seenIds.delete(s), 60*60*1000); return false; }
...
function toB64(obj:any){ return (globalThis as any).btoa(unescape(encodeURIComponent(JSON.stringify(obj)))); }
function fromB64<T=any>(b64:string): T {
  return JSON.parse(decodeURIComponent(escape((globalThis as any).atob(b64))));
}
...
export function stopP2P(){
  running = false;
  (globalThis as any).clearTimeout(dutyTimer);
...
async function handleDevice(dev: any){
  ...
  try{
    ...
    services.find((s: any)=>s.uuid.toUpperCase().includes(P2P_SERVICE.replace(/-/g,'')));
    ...
    try{
      ...
      // Ignore characteristic read errors
    }
    if(peerAggregate){
      ...
      if((bundle as any).kind && ((bundle as any).kind.startsWith('fam_'))){
        // Family message handling - placeholder for future implementation
      }
      ...
      if((bundle as any).kind==='pack_chunk' && (bundle as any).id && ((bundle as any).idx!=null) && (bundle as any).b64){
        ...
        (globalThis as any).setTimeout(()=> requestMissing((bundle as any).id).catch(()=>{}), 1000);
      }
      ...
      if((bundle as any) && (bundle as any)._from){ 
        if(!seenPeers.has((bundle as any)._from)){ 
          ...
          (globalThis as any).setTimeout(()=>{ seenPeers.delete((bundle as any)._from); setNeighborCount(seenPeers.size); }, 60_000); 
        } 
      }
...
async function scanOnce(){
  await advertiseHello(); // not used directly, but keeps meta updated
  const duty = getDuty();
...
async function cycle(){
  if(!running) {return;}
  await scanOnce();
  dutyTimer = (globalThis as any).setTimeout(()=>cycle(), getDuty().bleSleepMs);
}
