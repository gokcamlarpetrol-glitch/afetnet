...
export async function start(events: Events){
  ...
  const loop = async()=>{
    while(scanning){
      ...
      await new Promise(r=>(globalThis as any).setTimeout(r, dutyCycle.scanMs));
      ...
      if (relayMode){
        ...
        if (m){
          ...
          for (let i=0;i<total;i++){
            ...
            await new Promise(r=>(globalThis as any).setTimeout(r, 160));
          }
      ...
      await new Promise(r=>(globalThis as any).setTimeout(r, dutyCycle.pauseMs));
    }
  };
  loop();
}
...
export async function broadcastSOS(getBatt:()=>number, statuses: string[] = []){
  try{
    ...
    (globalThis as any).setTimeout(()=>{ Native.stopAdvertise().catch(()=>{}); }, 10000);
  }catch{
    // Ignore advertise errors
  }
}
...
export async function broadcastText(text: string){
  ...
  for (let i=0;i<total;i++){
    ...
    await Native.advertise(mask(pkt));
    await new Promise(r=>(globalThis as any).setTimeout(r, 200));
  }
  (globalThis as any).setTimeout(()=>{ Native.stopAdvertise().catch(()=>{}); }, 2000);
}
...
export async function broadcastTeamLocation(){
  try{
    ...
    await Native.advertise(mask(pkt));
    await new Promise(r=>(globalThis as any).setTimeout(r, 800));
    await Native.stopAdvertise();
  }catch{
