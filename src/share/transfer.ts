...
export async function senderThrottle(){ const p=getPower(); const pause = Math.max(150, Math.min(1200, Math.round(p.meshPingSec*25))); await new Promise(r=>(globalThis as any).setTimeout(r, pause)); }
