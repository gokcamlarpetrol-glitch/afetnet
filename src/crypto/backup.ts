...
function toU8(x:string){ return new Uint8Array(Buffer.from(x, "base64")); }
function b64(u8:Uint8Array){ return Buffer.from(u8).toString("base64"); }

// PBKDF2-HMAC-SHA256 -> keyLen bytes
export async function pbkdf2(passUtf8: Uint8Array, salt: Uint8Array, iter=100_000, keyLen=32){
  ...
  for(let i=1; out.length<keyLen; i++){
    ...
    const macHex = await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, Buffer.from(concat(salt,msg)).toString("hex"));
    last = new Uint8Array(Buffer.from(macHex,"hex"));
    out = concat(out, last);
  }
...
export async function secretboxEncrypt(plaintext: Uint8Array, password: string, salt?: Uint8Array){
  ...
  const key = await pbkdf2(new TextEncoder().encode(password), s, 100000, 32);
...
export async function secretboxDecrypt(obj:{salt_b64:string;nonce_b64:string;ct_b64:string}, password:string){
  const key = await pbkdf2(new TextEncoder().encode(password), toU8(obj.salt_b64), 100000, 32);
