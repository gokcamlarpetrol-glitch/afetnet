...
export async function createSessionFromBundles(threadId:string, myPre:{sk:Uint8Array; pk:Uint8Array}, peer:{ik_pub:Uint8Array; spk_pub:Uint8Array}){
  ...
  const s: Session = {
    threadId,
    our_sk_b64: Buffer.from(rk.secretKey).toString("base64"),
    our_pk_b64: Buffer.from(rk.publicKey).toString("base64"),
    their_pk_b64: Buffer.from(peer.spk_pub).toString("base64"),
    ck_s_b64: Buffer.from(ck_s).toString("base64"),
    ck_r_b64: Buffer.from(ck_r).toString("base64"),
...
export async function ratchetEncrypt(s: Session, plaintext: Uint8Array){
  ...
  const ck_s = b64(s.ck_s_b64); const mk = await kdfChain(ck_s);
  ...
  s.ck_s_b64 = Buffer.from(nextCk).toString("base64"); s.pn++;
  return { s, nonce: Buffer.from(nonce).toString("base64"), ct: Buffer.from(ct).toString("base64"), dh_pub_b64: s.our_pk_b64, pn: s.pn };
}

export async function ratchetDecrypt(s: Session, dh_pub_b64: string, nonce_b64: string, ct_b64: string){
  const theirPub = new Uint8Array(Buffer.from(dh_pub_b64,"base64"));
  ...
  if(dh_pub_b64 !== s.their_pk_b64){
    ...
    s.our_sk_b64 = Buffer.from(newOur.secretKey).toString("base64");
    s.our_pk_b64 = Buffer.from(newOur.publicKey).toString("base64");
    s.ck_r_b64 = Buffer.from(await hkdf(base, newOur.publicKey, "ck_r", 32)).toString("base64");
    s.ck_s_b64 = Buffer.from(await hkdf(base, theirPub, "ck_s", 32)).toString("base64");
    s.pn=0; s.rn=0;
  }
  const ck_r = b64(s.ck_r_b64);
  const mk = await kdfChain(ck_r); // symmetric step
  const nonce = new Uint8Array(Buffer.from(nonce_b64,"base64"));
  const ct = new Uint8Array(Buffer.from(ct_b64,"base64"));
  const plain = nacl.box.open(ct, nonce, new Uint8Array(Buffer.from(s.their_pk_b64,"base64")), b64(s.our_sk_b64));
  if(!plain) {return { ok:false as const, s };}
  const nextCk = await kdfChain(ck_r); s.ck_r_b64 = Buffer.from(nextCk).toString("base64"); s.rn++;
