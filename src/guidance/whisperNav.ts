...
export async function stopWhisper(){ try{ await snd?.stopAsync(); await snd?.unloadAsync(); }catch{} snd=null; on=false; }

export async function pulseHaptic(seq: "slow"|"fast"="slow"){
  const times = seq==="slow"? 3 : 6;
  for(let i=0;i<times;i++){ await Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium); await new Promise(r=>setTimeout(r, seq==="slow"? 800: 300)); }
}
